...
- `master` is clean and matches `origin/master` at `74190b5`.
- The latest `master` GitHub Actions run passed.
- The local Swift Testing suite passes with 98 tests on `master`; the isolated toolchain branch also passes with Swiftly-managed Swift 6.3.3.
- Live OmniFocus integration tests require their explicit environment flags and are not covered by the default test run.
...
- [ ] Replace stale entries in `NEXT-STEPS.md` with current issue links or make this file the canonical roadmap index.
- [ ] Retire or rewrite stale `SESSION_NOTES.md` claims about pending working-tree changes.
- [ ] Populate the `CHANGELOG.md` Unreleased section and remove the duplicate `1.0.0` entry.
...
- [ ] **P1** [#71](https://github.com/deverman/FocusRelayMCP/issues/71): make mutation save and per-target failures impossible to report as success. A scoped fix is implemented in local branch `fix/p1-mutation-save-truth`; it still needs branch publication, CI, and the issue's release validation before closure.
- [ ] **P2** [#61](https://github.com/deverman/FocusRelayMCP/issues/61): align project available-task counts with shared task availability semantics. The isolated `fix/project-count-parity` branch now has documented bridge/JXA semantics, root-task exclusion for project-scoped task parity, unit/live tests, a passing semantic gate, and a clean 10-minute smoke. It still needs publication, integration, and the required 1-hour realistic validation before closure.
- [ ] **P2** [#64](https://github.com/deverman/FocusRelayMCP/issues/64): fix CI artifact upload, manual release version selection, and prerelease metadata. A scoped fix is implemented and locally validated on branch `fix/release-packaging`; it still needs publication and CI.
- [ ] **P2** [#72](https://github.com/deverman/FocusRelayMCP/issues/72): remove the stale checked-in formula and validate the authoritative tap at `/Users/deverman/Documents/code/homebrew-focus-relay`. The ownership/docs/script changes are implemented on branch `fix/homebrew-formula-ownership`; two formula style defects are fixed locally in the tap checkout. Both repositories still need publication and CI/tap validation.
- [ ] **Release prerequisite** [#58](https://github.com/deverman/FocusRelayMCP/issues/58): unify CLI, MCP, and release version metadata. A scoped implementation on branch `fix/version-reporting` has one embedded version source, `focusrelay --version`, matching MCP metadata, semantic-version injection, tests, and docs; it still needs integration with #64, publication, and CI.
...
- [ ] Publish and merge the isolated toolchain branch before release validation.
...
1. Finish the stale PR and reliability audit.
2. Clean up GitHub issue/PR state and establish a release milestone.
3. Prepare and validate the unreleased mutation/folder-discovery release.
4. Implement #11, then #22, then #18 as separate branches.
5. Add native task-status filters and general sorting as separate branches.
6. Run a documented feasibility phase before implementing #10 custom perspectives.
7. Re-scope #16 after verifying the current official planned-date setter contract.
