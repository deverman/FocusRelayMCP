...
The active slice is [#94 — refine the shipped `process_inbox`
workflow](https://github.com/deverman/FocusRelayMCP/issues/94). It adds
GTD-informed clarification, preserves conservative ambiguity and bounded
lookup behavior, and exposes one shared workflow definition through MCP and
the CLI. Additional workflow prompts still require user evidence.

## Delivery Order

1. [#94 — GTD-informed `process_inbox` and shared CLI/MCP workflow](https://github.com/deverman/FocusRelayMCP/issues/94)
   - Clarify or preserve vague captures; never infer drop from ambiguity.
   - Share one workflow definition between MCP and CLI without adding a tool.
2. [#92 — guided Homebrew setup](https://github.com/deverman/FocusRelayMCP/issues/92)
...
3. [#85 — truthful Forecast contract](https://github.com/deverman/FocusRelayMCP/issues/85)
...
4. [#11 — OmniFocus URLs](https://github.com/deverman/FocusRelayMCP/issues/11)
   - Add an optional native `url` field to existing task and project outputs;
     do not add a tool.
5. [#82 — safe task/subtask creation](https://github.com/deverman/FocusRelayMCP/issues/82)
